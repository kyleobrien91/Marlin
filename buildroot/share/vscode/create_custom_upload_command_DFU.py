...
if build_type not in ['upload', 'traceback', 'Not Set']:
  ...
  if current_OS == 'Windows':
    ...
    upload_string = f'avrdude -p usb1286 -c flip1 -C {avrdude_conf_path} -U flash:w:{source_path}:i'

  else:
    source_path = env.get("PROJECTBUILD_DIR") + '/' + env.get("PIOENV") + '/firmware.hex'

    upload_string = f'avrdude -p usb1286 -c flip1 -U flash:w:{source_path}:i'
